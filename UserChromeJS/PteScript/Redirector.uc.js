...
//			XULBrowserWindow.statusTextField.label = "Redirector 规则已重新载入";
		...
		drawUI: function() {
			if (this.addIcon && !document.getElementById("redirector-icon")) {
				...
				let icon;
				if (this.type == 0) {
					icon = document.getElementById("TabsToolbar_aidBar").appendChild(document.createElement("toolbarbutton"));
				...
				icon.setAttribute("style", "padding: 5px; list-style-image: url(" + (this.state ? this.enableIcon : this.disableIcon) + ")");
				// add menu
				let xml = '\
